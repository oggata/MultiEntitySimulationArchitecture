...
# ============================================================
# 📸 画像処理パラメータ
MAX_IMAGE_SIZE = 800      # 入力画像の最大サイズ (640-2048) ⬆️大きいほど精度向上
USE_TILING = True          # タイル分割処理（精度向上に重要）
TILE_SIZE = 320            # タイルサイズ (120-640) ⬆️大きいほど精度向上
TILE_OVERLAP = 64          # タイル間の重複 (32-128) ⬆️大きいほど境界が綺麗

# 🏗️ メッシュ生成パラメータ
MIN_SEGMENT_AREA = 32      # 最小セグメント面積 (20-200) ⬆️増やすとセグメント数減少
MESH_RESOLUTION = 3        # メッシュ解像度 (1-4、大きいほど粗くて軽量) ⬆️増やすとファイルサイズ減少
...
BOUNDARY_THICKNESS = 5     # 境界の太さ（ピクセル）1-5 ⬇️小さくすると建物と道路の混合を軽減
